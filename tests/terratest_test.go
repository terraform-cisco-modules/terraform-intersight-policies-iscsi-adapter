...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-iscsi-a-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} iSCSI Adapter Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/vnic/IscsiAdapterPolicies/%s", moid), expectedJSONTemplate, vars)
}
